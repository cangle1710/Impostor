...
export default function OnlineDiscussionPage() {
  const { players, settings, round, myRole, isHost, accuse, submitImpostorGuess, revealResults, leaveLobby } = useLobby();

  const endsAt = round?.discussionEndsAt ?? null;
  const accusation = round?.accusation ?? null;
  const accusationDone = accusation !== null;
  ...
  return (
    <div className="min-h-dvh bg-bg flex flex-col px-4 py-8 safe-pt safe-pb">
      <div className="w-full max-w-sm mx-auto flex flex-col gap-6">
        ...
        {accusation && !accusation.correct && (
          <div className="bg-orange-500/10 border border-orange-500/40 rounded-2xl px-4 py-3 text-center">
            <p className="text-orange-300 font-semibold text-sm">
              Wrong! {accusation.accusedName} is innocent.
            </p>
        ...
        {settings?.allowImpostorGuess && isImpostor && !guessSubmitted && (
          showGuessForm ? (
            <form onSubmit={handleGuessSubmit} className="bg-[#1e1640] border border-purple-500/40 rounded-2xl p-4 flex flex-col gap-3">
              <p className="text-purple-300 text-sm font-semibold">Your word guess</p>
              <input
                type="text"
                value={guessInput}
